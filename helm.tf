...
resource "helm_release" "nginix-ingress" {
 ...
 resource "null_resource" "get_nlb_hostname" {
   ...
   provisioner "local-exec" {
     command = "aws eks update-kubeconfig --name Alt-eks --region eu-west-2 && kubectl get svc nginix-ingress-ingress-nginx-controller  --namespace ingress -o jsonpath='{.status.loadBalancer.ingress[*].hostname}' > ${path.module}/lb_hostname.txt"
   }
 }
